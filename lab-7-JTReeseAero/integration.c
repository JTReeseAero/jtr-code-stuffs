...
double midpoint(double (*func)(double), double a, double b){
  ...
  double solution = (b-a)*func((a+b)/2);
  return solution;

}//end midpoint method

double simpson_13(double (*func)(double), double a, double b){
  double solution = ((b-a)/6) * (func(a) + 4*func((a+b)/2) + func(b));
  return solution;

}//End Simpson 1/3 method

double simpson_38(double (*func)(double), double a, double b){
  double solution = ((b-a)/8) * (func(a) + 3*func((2*a+b)/3) + 3* func((a+2*b)/3) + func(b));
  return solution;
...
double gauss_quad (double (*func)(double), double a, double b, int n){
  ...
  for (i = 0; i < n; i++){
    solution += w[n-2][i] * func(c + m*t[n-2][i]);    
  }
  solution = solution * m;
  return solution;
